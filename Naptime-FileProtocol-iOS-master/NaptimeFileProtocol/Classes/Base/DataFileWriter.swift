...
open class DataFileWriter: DataFileWritable {
    ...
    open let protocolVersion: String = "1.0"
    open let headerLength: Byte = 32
    ...
    open private (set) var timestamp: Timestamp = Timestamp(NSDate().timeIntervalSince1970)
    ...
    open func createFile(_ fileURL: URL) throws {
        ...
        do {
            let tempPath = NSTemporaryDirectory() + fileURL.lastPathComponent
            self.tempURL = URL(fileURLWithPath: tempPath)
            #if swift(>=4.0)
...
//        self.data.append(data)
    ...
    fileprivate func mergeHeader() -> Data {
        var header = [Byte]()
        ...
        return Data(bytes: UnsafePointer<UInt8>(header), count: header.count)
    }
